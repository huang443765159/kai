...
import threading
import time
from CarGuide.CARGUIDE.DRIVER.AutoNET.Socket._Tool import get_my_ip
...
class UDP(object):
    ...
    def _working(self):
        ...
        while 1:
            if self._is_bound:
                ...
                # INVITER
                if self._is_bound and self._invite_type is not None:
                    if time.time() - self._invite_ts > self._invite_period:
                        invite_info = b'\x99\x00'
                        desc = f'{self.get_my_ip()}|{self._invite_type}_{self._invite_id}'
                        ...
                        self._event_cb(module='ANET_INVITER', code='INVITE_SENT',
                                       value=(self._invite_type, self._invite_id))
            else:
                # BIND
                self._udp.close()
                self._udp = socket.socket(family=socket.AF_INET, type=socket.SOCK_DGRAM)
    ...
    # INVITE
    def set_invite_type(self, device_type):
        self._invite_type = device_type

    def set_invite_id(self, device_id):
        self._invite_id = device_id
...
if __name__ == '__main__':
    ...
    while 1:
        # udp.send_to(b'\xff\x00\x00\xaa', ip='10.0.1.3')
        udp.send_broadcast(b'\x88\x00')
        time.sleep(1)
