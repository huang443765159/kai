...
class Drain(QObject):
    ...
    def __init__(self, pi_gpio, pins=(17, 18)):
        super().__init__()
        # DRAIN
        self._drain = dict()
        self._row_data = {0: False, 1: False}
        for sid, pin in enumerate(pins):
            self._drain[sid] = OneSensor(pi_gpio=pi_gpio, sid=sid, pin=pin, data_cb=self._data_callback)

    def _data_callback(self, sid, data):
        if self._row_data[sid] != data:
            self._row_data[sid] = data
            self.sign_drain_raw_data.emit(list(self._row_data.values()))

    def exit(self):
        for drain in list(self._drain.values()):
            drain.exit()
